...
res1 <- results(dds, contrast=c("type","ABMR", "STA"), alpha = .05) #length(which(res1$padj<0.05)) 5,491
res2 <- results(dds, contrast=c("type","TCMR","STA"), alpha = .05) #length(which(res2$padj<0.05)) 0
res3 <- results(dds, contrast=c("type","ABMR","TCMR"), alpha = .05) #length(which(res3$padj<0.05)) 2,442
...
results_STA_AMR_DEseq<-annotation_sign #5491
results_STA_AMR_DEseq_FC<-results_STA_AMR_DEseq[which(abs(results_STA_AMR_DEseq$log2FC)>0.585),] #4790
...
plot_heatmap(clin,norm_data_rlog, "ABMR", "STA", results_STA_AMR_DEseq_FC, color=COLOR[c(3,1)])
dev.off()

#2. ENET with binomial distribution
results_STA_AMR_ENET<-ENET_binomial(clin, "ABMR", "STA", norm_data_rlog) ##59 genes
...
plot_heatmap(clin,norm_data_rlog, "ABMR", "STA", results_STA_AMR_ENET, color=COLOR[c(3,1)])
dev.off()

id_overlap<-match(results_STA_AMR_ENET$name,results_STA_AMR_DEseq$name)
length(which(is.na(id_overlap)==FALSE))

###cluster and similarity
results_STA_AMR_ENET<-read.csv("Results/RNAseq/results_STA_AMR_DEseq.csv")
...
boxplot(norm_data_rlog[id,which(clin!="ABMR")]~factor(clin[which(clin!="ABMR")]),col=COLOR[c(3,2)],ylab="TCAF1 log2 gene expression")
dev.off()
...
results_CMR_AMR_DEseq<-annotation_sign #2242
results_CMR_AMR_DEseq_FC<-results_CMR_AMR_DEseq[which(abs(results_CMR_AMR_DEseq$log2FC)>0.585),] #2100
...
###Plot the results
tiff(filename = "Results/RNAseq/heatmap_CMR_AMR_Dseq2_FC.tiff", width = 4000, height = 3000,  res = 300)
plot_heatmap(clin,norm_data_rlog, "ABMR", "TCMR", results_CMR_AMR_DEseq_FC, color=COLOR[c(2,1)])
dev.off()

#2. ENET with binomial distribution
results_CMR_AMR_ENET<-ENET_binomial(clin, "ABMR", "TCMR", norm_data_rlog) ##23 genes
...
plot_heatmap(clin,norm_data_rlog, "ABMR", "TCMR", results_CMR_AMR_ENET, color=COLOR[c(2,1)])
dev.off()
...
###cluster and similarity
results_CMR_AMR_ENET<-read.csv("Results/RNAseq/results_CMR_AMR_DEseq.csv")
...
results_STA_REJ_DEseq<-annotation_sign #1385
results_STA_REJ_DEseq_FC<-results_STA_REJ_DEseq[which(abs(results_STA_REJ_DEseq$log2FC)>0.585),] #1179
...
plot_heatmap(clin2,norm_data_rlog, "REJ", "STA", results_STA_REJ_DEseq, color=COLOR[c(3,1)])
dev.off()
...
write.csv(cbind(results,coef1,coef2),"Results/RNAseq/results_AMR_CMR_STA_ENET.csv",row.names = F) #102 
...
COLOR = brewer.pal(4,"Pastel1")
ann_colors = list(Type = c("ABMR" = COLOR [1] ,"TCMR" = COLOR[2], "STA" = COLOR[3]))
